...
main!.innerHTML += `
    ...
    `
const pp: HTMLDivElement | null = document.querySelector(".home_img")
// appear(pp!)



//? BOUTONS LIENS HOME VERSION DESKTOP
const homeTxt: HTMLDivElement | null = document.querySelector('.home_txt')
...
const P = new Project()
P.getProjects().then(() => {
    const projectSection: HTMLDivElement | null = document.querySelector("#projects");
    const projects: HTMLDivElement[] | null = Array.from(document.querySelectorAll(".projectContainer"))
    // console.log(projects);
    const observer = new IntersectionObserver(setIndicator, { threshold: 0.05 });
    observer.observe(projectSection!)
    let lastScrollY = window.scrollY
    function setIndicator(entries: IntersectionObserverEntry[]) {
...
const mail: HTMLInputElement | null = document.querySelector(".mailInput")
const comment: HTMLInputElement | null = document.querySelector(".msgInput")

//* fonction "squelette" pour comparer l'input des utilisateurs et la regex, et associer un code couleur en cas d'erreur
function compareInputToRegex(inputHTLM: HTMLInputElement, inputRegex: RegExp): void {
...
function RegexOnFillIn() {
    const fields: { input: HTMLInputElement | null, regex: RegExp }[] = [
...
//? ANIMATIONS 
//* Hellothere
const title: HTMLHeadingElement | null = document.querySelector('.helloThere')
const homeBtns: HTMLElement[] | null = Array.from(document.querySelectorAll('.btn'))
...
//* appear
function appear(e: HTMLElement, scrollingDown: boolean = true) {
    let keyframes
    if (scrollingDown) {
        keyframes = {
            opacity: ["0", "0.5", "1"],
